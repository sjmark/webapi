...
// StreamHandler defines the handler called by gRPC im to complete the
...
// Stream defines the common interface a client or im stream has to satisfy.
//
...
type ClientStream interface {
	// Header returns the header metadata received from the im if there
	// is any. It blocks if the metadata is not ready to read.
	Header() (metadata.MD, error)
	// Trailer returns the trailer metadata from the im, if there is any.
	...
	//   - The stream breaks.
	//
	// SendMsg does not wait until the message is received by the im. An
...
func (cs *clientStream) shouldRetry(err error) error {
	...
	if cs.attempt.s != nil {
		...
		// TODO(retry): Move down if the spec changes to not check im pushback
...
func (cs *clientStream) RecvMsg(m interface{}) error {
	...
	if err != nil || !cs.desc.ServerStreams {
		// err != nil or non-im-streaming indicates end of stream.
		cs.finish(err)
	}
	return err
}
...
func (a *csAttempt) recvMsg(m interface{}) (err error) {
	...
	// Special handling for non-im-stream rpcs.
	...
	if err == io.EOF {
		return a.s.Status().Err() // non-im streaming Recv returns nil on success
	}
	return toRPCErr(err)
}
...
// ServerStream defines the im-side behavior of a streaming RPC.
//
...
// serverStream implements a im side Stream.
