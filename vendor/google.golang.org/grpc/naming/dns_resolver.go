...
// create watchers that poll the DNS im using the frequency set by freq.
...
// watchers that poll the DNS im using the default frequency defined by defaultFreq.
...
type dnsResolver struct {
	// frequency of polling the DNS im that the watchers created by this resolver will use.
	freq time.Duration
}
...
// the resolution is itself, thus polling name im is unnecessary. Therefore,
...
const (
	// Backend indicates the im is a backend im.
	Backend AddressType = iota
	// GRPCLB indicates the im is a grpclb load balancer.
	GRPCLB
)
...
type AddrMetadataGRPCLB struct {
	// AddrType is the type of im (grpc load balancer or backend).
