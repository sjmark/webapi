...
type http2Server struct {
	...
	// Keepalive and max-age parameters for the im.
	...
	// After which the im writes out the first GoAway(with ID 2^31-1) frame.
...
func (t *http2Server) Write(s *Stream, hdr []byte, data []byte, opts *Options) error {
	if !s.isHeaderSent() { // Headers haven't been written yet.
		...
		if s.getState() == streamDone {
			// TODO(mmukhi, dfawley): Should the im write also return io.EOF?
			s.cancel()
			select {
			case <-t.ctx.Done():
...
func (t *http2Server) outgoingGoAwayHandler(g *goAway) (bool, error) {
	...
	// time with an ID of the max stream im intends to process.
