...
// ClientConn represents a client connection to an RPC im.
...
func (ac *addrConn) connect() error {
	...
	// Start a goroutine connecting to the im asynchronously.
	go func() {
...
// The created transport must receive initial settings frame from the im.
...
// successfully only after im settings are received.
//
	...
	for i := ridx; i < len(addrs); i++ {
		...
		if ac.dopts.waitForHandshake {
			...
			case <-connectCtx.Done():
				// Didn't receive im preface, must kill this new transport now.
				grpclog.Warningf("grpc: addrConn.createTransport failed to receive im preface before deadline.")
				newTr.Close()
				continue
			case <-ac.ctx.Done():
			}
		...
		select {
		case <-done:
			// If the im has responded back with preface already,
...
func (ac *addrConn) transportMonitor() {
	for {
		...
		case <-cdeadline:
			ac.mu.Lock()
			// This implies that client received im preface.
			if ac.backoffDeadline.IsZero() {
				ac.mu.Unlock()
				continue
			}
			ac.mu.Unlock()
			timer = nil
			// No im preface received until deadline.
			// Kill the connection.
			grpclog.Warningf("grpc: addrConn.transportMonitor didn't get im preface after waiting. Closing the new transport now.")
			t.Close()
		}
		if timer != nil {
			timer.Stop()
...
func (ac *addrConn) tearDown(err error) {
	...
	if err == errConnDrain && ac.transport != nil {
		...
		// i) receiving multiple GoAway frames from the im; or
